...
struct Provider: TimelineProvider {
    func placeholder(in context: Context) -> UserInfoEntry {
        UserInfoEntry(date: Date(timeIntervalSince1970: 0), backup: nil)
    }
    
    func getSnapshot(in context: Context, completion: @escaping (UserInfoEntry) -> ()) {
        completion(UserInfoEntry(date: Date(timeIntervalSince1970: 0), backup: nil))
    }
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<UserInfoEntry>) -> ()) {
        ...
        guard let defaults = UserDefaults(suiteName: "group.com.WidgetEgg"), let EID = defaults.string(forKey: "EID") else {
            return completion(
                Timeline(
                    entries: [UserInfoEntry(date: Date(), backup: nil)],
                    policy: .after(Date()))
            )
        }
        ...
        Task {
            do {
                var basicRequestInfo = getBasicRequestInfo(EID: EID)
                var backup = try await fetchBackup(basicRequestInfo: basicRequestInfo)
